                        ...
                        {cart?.map((p) => (
                            <div className="row m-2 card flex-row rounded-3" key={p._id}>
                                <div className="col-md-4">
                                    <img src={`http://localhost:8080/api/v1/product/product-photo/${p._id}`} alt={p.name} style={{ width: "270px", height: "270px", margin: "10px", borderRadius: "5px" }} />
                                </div>
                                <div className="col-md-8 px-5">
                                    <h6 className='mt-5'> Name : {p.name}</h6>
                                    <p>Description : {p.description.substring(0, 30)}</p>
                                    <p>Price : {p.price}</p>
                                    <button className='btn btn-danger btn-sm px-3 rounded-3' onClick={() => { removeCartItem(p._id) }}> Remove </button>
                                </div>
                            </div>
                        ))}
                    </div>
                            ...
                            <div className="mb-3">
                                {
                                    auth?.token ? (<button className='btn btn-outline-warning btn-sm' onClick={() => navigate('/dashboard/user / profile')}> Update Address</button>) :
                                        (<button className='btn btn-outline-warning btn-sm' onClick={() => navigate('/login', { state: '/cart', })}> Please login to Checkout</button>)
                                }
                            </div>
                        )}
                        <div className="mt-3">
